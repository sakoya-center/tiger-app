...
export default function Home() {
  ...
  const exportZip = async (receiptsList: any[], label: string, isNewReport: boolean) => {
    setIsExporting(true);
    setMessage({ text: "אורז קבלות ומכין קובץ, נא להמתין... 📦", type: "success" });
    try {
      const zip = new JSZip();
      ...
      let csvContent = "\uFEFFשם ספק,סכום כולל,מעמ,תאריך,ח.פ,מספר קבלה,קטגוריה\n";

      for (let i = 0; i < receiptsList.length; i++) {
        const r = receiptsList[i];
        csvContent += `"${r.supplierName || ""}","${r.totalAmount || ""}","${r.vatAmount || ""}","${r.date || ""}","${r.businessId || ""}","${r.invoiceNumber || ""}","${r.category || ""}"\n`;
